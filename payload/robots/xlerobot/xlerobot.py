...
class XLerobot(Robot):
    ...
    def __init__(self, config: XLerobotConfig):
        ...
        # Last known-good head positions, used as a fallback when the head motors
        # (ids 7/8, sharing bus1 with the left arm) drop a sync_read/sync_write so a
        # flaky connection doesn't crash the whole teleop loop over a non-critical joint.
        self._last_head_pos: dict[str, float] = {}
    ...
    def _read_head_pos_soft(self) -> dict[str, float]:
        """Read head motor (ids 7/8) positions, tolerating a flaky bus.
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.config.max_relative_target is not None:
            ...
            present_pos_head = self._read_head_pos_soft()
        ...
        if head_pos_raw:
            try:
                ...
                logger.warning(f"{self}: head motors (ids 7/8) unreachable, skipping head move: {e}")
